...
Bei Strong Scaling (oben) bleibt die Gittergrösse gleich, und nur die Thread-Zahl steigt — jeder
Thread bekommt dabei immer weniger Arbeit. Bei Weak Scaling ist es umgekehrt: Die Gittergrösse
wächst mit der Thread-Zahl mit, sodass jeder Thread ungefähr gleich viel Arbeit behält (hier:
~40'000 Zellen pro Thread; `nx` wächst proportional zur Thread-Zahl, `ny` und die Schrittzahl
bleiben fix). Bei perfekter Skalierung bliebe die Laufzeit dabei über alle Thread-Zahlen hinweg
konstant.

| Threads | Gitter (nx×ny) | Laufzeit | Effizienz | MLUPS |
|---|---|---|---|---|
| 1  | 400×100   | 3.53 s   | 100.0% | 22.64 |
| 2  | 800×100   | 4.23 s   | 83.5%  | 37.86 |
| 4  | 1600×100  | 7.99 s   | 44.2%  | 40.07 |
| 8  | 3200×100  | 18.63 s  | 18.9%  | 34.34 |
| 16 | 6400×100  | 74.83 s  | 4.7%   | 17.11 |
| 24 | 9600×100  | 116.71 s | 3.0%   | 16.45 |
| 32 | 12800×100 | 153.07 s | 2.3%   | 16.72 |

![Weak Scaling](figures/weak_scaling.png)

In der Praxis sieht es deutlich schlechter aus als das ideale Bild: Statt konstant zu bleiben,
steigt die Laufzeit steil an, und die Effizienz fällt schon ab 8 Threads unter 20%. Das passt zum
Bild aus Abschnitt 7 und 8.1: Der Kernel ist speicherbandbreiten-limitiert, nicht
kernanzahl-limitiert. Bei Weak Scaling wächst mit der Thread-Zahl auch die Datenmenge, die
gleichzeitig durch den Speicherbus muss — und weil sich alle Threads dieselbe Speicherbandbreite
teilen, bringt ein zusätzlicher Thread ab einem gewissen Punkt keinen zusätzlichen Durchsatz mehr,
sondern nur noch mehr Konkurrenz um dieselbe Bandbreite. Der MLUPS-Wert bestätigt das direkt: Er
steigt bis 4 Threads (40.07 MLUPS), fällt danach aber wieder — bei mehr Threads wird pro Sekunde
insgesamt weniger Arbeit erledigt, nicht mehr, obwohl jeder einzelne Thread gleich viel zu tun hat.
